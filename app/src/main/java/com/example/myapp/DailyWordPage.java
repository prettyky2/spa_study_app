...
public class DailyWordPage extends AppCompatActivity implements View.OnClickListener {
    ...
    private void showCustomDialog(int chunkIndex) {
        ...
        dialog.setContentView(R.layout.activity_daily_word_dialog);
    ...
    private void navigateToNextActivity(int chunkIndex, int mode) {
        Intent intent = new Intent(this, DailyWordStudyTest.class); // NextActivity는 대상 액티비티

        // 선택된 chunk 데이터 가져오기
        List<Row> selectedChunk = chunkedRows.get(chunkIndex);

        // Row 데이터를 ExcelRow DTO로 변환
        List<ExcelRow> dtoList = new ArrayList<>();
        for (Row row : selectedChunk) {
            ...
            dtoList.add(new ExcelRow(column1, column2, column3, column4));
        }
